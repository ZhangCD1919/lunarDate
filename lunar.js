/**
 * Created by Zhangcaidie on 2018/5/7.
 */

function lunarFigure(timeStamp) {
    let lunarInfo = [
    ...
    let SY = new Date(date).getYear();
    let SM = new Date(date).getMonth();
    let SD = new Date(date).getDate();
    ...
    this.AnmailYear = function() {
        let sDObj = new Date(SY, SM, SD);
        let lDObj = new this.Lunar(sDObj);
        console.log(lDObj);
        return Animals[(lDObj.year - 4) % 12];
    };
...
//==== 中文日期
    ...
    function solarDay1() {
        ...
        let tt = '【' + Animals[(SY - 4) % 12] + '】' + cyclical(lDObj.monCyl) + '月 ' + cyclical(lDObj.dayCyl++) + '日';
        return (tt);
    }
    function solarDay2() {
        let sDObj = new Date(SY, SM, SD);
        ...
        let tt = cyclical(SY - 1900 + 36) + '年 ' + cDay(lDObj.month, lDObj.day);
        return (tt);
    }
    function solarDay3() {
        ...
        let lFtv = ["0101*春节", "0115 元宵节", "0505 端午节", "0707 七夕情人节", "0715 中元节", "0815 中秋节", "0909 重阳节", "1208 腊八节", "1224 小年", "0100*除夕"];
        let sFtv = ["0101*元旦", "0214 情人节", "0308 妇女节", "0309 偶今天又长一岁拉", "0312 植树节", "0315 消费者权益日", "0401 愚人节", "0418 MM的生日", "0501 劳动节", "0504 青年节", "0512 护士节", "0601 儿童节", "0701 建党节 香港回归纪念", "0801 建军节", "0808 父亲节", "0909 毛席逝世纪念", "0910 教师节", "0928 孔子诞辰", "1001*国庆节",
...
lunarFigure.lunarDate = function (timeStamp) {
    return new lunarFigure(timeStamp).lunarDate();
};
